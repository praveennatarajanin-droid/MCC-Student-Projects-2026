...
exports.createWorker = async (req, res) => {
  try {
    ...
    const { name, age, phoneNo, email, address, role, store, salary, unit, aadharNo } = req.body;
    const actualStore = store || unit;

    if (!name || !age || !phoneNo || !email || !address || !role || !actualStore || !salary || !aadharNo) {
...
exports.updateWorker = async (req, res) => {
  try {
    const { phoneNo, email, salary, aadharNo, unit, store } = req.body;
    ...
    if (aadharNo) {
      ...
      req.body.aadharNo = aadharNoClean;
    }

    // Map unit to store if store is not provided
    if (unit && !store) {
      req.body.store = unit;
    }
